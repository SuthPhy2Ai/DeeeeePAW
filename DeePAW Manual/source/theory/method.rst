...
原子 MPNN 负责学习原子级别的表征。初始边嵌入通过高斯基函数 (Gaussian Basis Function, GBF) 构建:
...
电子密度 MPNN 在网格点级别工作。原子 MPNN 的节点嵌入被馈入 ED MPNN，网格点通过聚合来自邻近原子核的信息来更新自身嵌入。嵌入更新过程中使用共享权重向量和交互权重向量来控制节点自身特征与邻居消息的相对贡献。

**输出头**

输出头包含三个模块:

- **GAT 头**: 基于图注意力网络 (Graph Attention Network)，从原子 MPNN 所有层的节点嵌入中提取形成能
- **MLP 头**: 从 ED MPNN 所有层的节点嵌入中生成平滑变化的电子密度轮廓
- **KAN 头**: 基于 Kolmogorov-Arnold 网络，从 ED MPNN 最后一层的节点嵌入中计算残差电子密度
...
- **第二阶段**: 冻结已训练的参数，单独训练 KAN 头以捕获残差电子密度的精细结构

训练数据来自 Materials Project (MP) 数据库，采用九折交叉验证。电子密度标签由 KSDFT 轨道波函数计算得到。
