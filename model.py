...
class E3DensityModel(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        return probe_result,prob_pep# atom_representation[-1] #prob_pep #atom_representation[-1]
...
class ResidualCorrectionModel(nn.Module):
    def __init__(self, num_embeddings, embedding_dim):
        super(ResidualCorrectionModel, self).__init__()
        
        self.embedding = nn.Embedding(num_embeddings=num_embeddings, embedding_dim=embedding_dim)
        # 修改线性层的输出维度为 12 以支持 1 到 6 次方
        self.mix = nn.Linear(embedding_dim, 12)
        nn.init.constant_(self.mix.weight, 0) 
        nn.init.constant_(self.mix.bias, 0)    
        self.a_w = nn.Parameter(torch.tensor(0.12, requires_grad=True))
        self.mlp = nn.Sequential(
            nn.Linear(992, 24),  # 第一层：从 992 -> 256
            # nn.Linear(992, 512),  # 第一层：从 992 -> 256
            # nn.ReLU(),                      # 激活函数
            # nn.Linear(512, 128),            # 第二层：从 256 -> 128
            # nn.ReLU(),                      # 激活函数
            # nn.Linear(128, 1)               # 第三层：从 128 -> 输出 dis_factor 的维度 1
        )
        self.kan = KAN(width=[24,8,4,1],grid=8,k=3,seed=42)
    def forward(self, input_dict,node_rep):
        
        #node_rep = node_rep
        #print("noderep",node_rep.shape)
        dis_factor_mlp = self.mlp(node_rep)

        # ele_emb = self.embedding(input_dict['neighbour_atom']).squeeze(0)
        # #print(node_rep.shape,ele_emb.shape)  # ([1400, 992]) torch.Size([1400, 128])
        # distance = input_dict['distances'].squeeze(0)
        # num_atoms = input_dict['num_neighbour_atoms'].long().squeeze()

        # ele_emb = torch.split(ele_emb, num_atoms.tolist())
        # ele_emb = torch.cat([torch.prod(item, dim=0).unsqueeze(0) for item in ele_emb], dim=0)
        # ele_emb = self.mix(ele_emb)

        # distance = torch.split(distance, num_atoms.tolist())
        # distance = torch.tensor([item.min() for item in distance], device=ele_emb.device)

        # # 修改公式，支持 1 到 6 次方
        # dis_factor = ele_emb[:, 0] * torch.exp(-1.0 * ele_emb[:, 1] * distance) + \
        #              ele_emb[:, 2] * torch.exp(-1.0 * ele_emb[:, 3] * distance * distance) + \
        #              ele_emb[:, 4] * torch.exp(-1.0 * ele_emb[:, 5] * distance * distance * distance) + \
        #              ele_emb[:, 6] * torch.exp(-1.0 * ele_emb[:, 7] * distance * distance * distance * distance) + \
        #              ele_emb[:, 8] * torch.exp(-1.0 * ele_emb[:, 9] * distance * distance * distance * distance * distance) + \
        #              ele_emb[:, 10] * torch.exp(-1.0 * ele_emb[:, 11] * distance * distance * distance * distance * distance * distance)




        #dis_factor_ = dis_factor
        dis_factor_ = self.kan(dis_factor_mlp)
        # dis_factor_ = dis_factor_mlp
        # dis_factor_ = self.a_w * dis_factor.unsqueeze(1)  + (1- self.a_w)*dis_factor_mlp
        dis_factor_far =  0

        return dis_factor_, dis_factor_far