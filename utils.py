...
class GraphConstructor(object):
    ...
    def sample_probes(self, grid_pos, density):
        if self.num_probes is not None:
            if isinstance(self.num_probes, list):
                # 如果 num_probes 是一个列表，则使用该列表中的索引来选择位置
                probe_choice = np.array(self.num_probes)  # 确保是一个 numpy 数组
                probe_choice = np.unravel_index(probe_choice, grid_pos.shape[0:3])  # 将一维索引转换为多维坐标
            else:
                # 如果 num_probes 是一个数字，按原方式进行随机选择
                probe_choice_max = np.prod(grid_pos.shape[0:3])
                probe_choice = np.random.randint(probe_choice_max, size=self.num_probes)
                probe_choice = np.unravel_index(probe_choice, grid_pos.shape[0:3])  # 将一维索引转换为多维坐标
            
            probe_pos = grid_pos[probe_choice]  # 根据索引获取位置
            probe_target = density[probe_choice]  # 根据索引获取目标值           
        else:
...
class MyCollator(object):
    ...
    def __call__(self, examples):
        ...
        # rows = [rows[ind_examples.index(i)] for i in range(len(rows))]
        list_of_dicts = []
        for row in rows:
            atoms = row.toatoms()
            atoms.set_pbc(True)
            ...
            graph_dict = self.graph_constructor.__call__(density=density, atoms=atoms, grid_pos=probe_pos)
