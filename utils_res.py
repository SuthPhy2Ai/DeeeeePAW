...
class GraphConstructor(object):
    ...
    def sample_probes(self, grid_pos, density, atoms, threshold_distance=1.0):
        ...
        probe_pos = probe_pos[valid_indices, :]
        probe_target = probe_target[valid_indices]
        if self.num_probes is not None:
            probe_choice_max = len(probe_target)
            probe_choice = np.random.randint(probe_choice_max, size=self.num_probes)
            probe_pos = probe_pos[probe_choice, :]
            probe_target = probe_target[probe_choice]
...
class MyCollator(object):
    ...
    def __call__(self, examples):
        ...
        for row in rows:
            ...
            graph_dict, probe_pos_chosen = self.graph_constructor.__call__(density=density, atoms=atoms, grid_pos=probe_pos, threshold_distance=threshold_distance)
            graph_dict = res_input_constructor(atoms=atoms, 
                                               grid_pos=probe_pos_chosen, 
                                               graph_dict=graph_dict, 
                                               threshold_distance=threshold_distance)
            list_of_dicts.append(graph_dict)

        return collate_list_of_dicts(list_of_dicts, pin_memory=False)


def closest_sum(nums, target):
    dp = {0: []}  # 使用字典来记录可能的和及其对应的索引列表

    for i, num in enumerate(nums):
        new_dp = dp.copy()
        for t, index_list in dp.items():
            new_sum = t + num
            if new_sum not in new_dp or len(new_dp[new_sum]) > len(index_list) + 1:
                new_dp[new_sum] = index_list + [i]
        dp = new_dp

    # 找到与目标值最接近的和
    closest_sum = min(dp.keys(), key=lambda x: abs(x - target))
    
    return dp[closest_sum]  # 返回最接近和的索引列表
